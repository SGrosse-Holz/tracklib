...
#         p_backward += np.log(scipy.stats.beta.pdf(cur_tau, betaParam*prop_tau / (1-prop_tau), betaParam))
    ...
    def logL(self, sequence):
        return rouse.likelihood(self.trace, sequence.toLooptrace(), self.model, **self.config['_logL_kwargs']) \
...
class LoopTraceMCMC(mcmc.Sampler):
    ...
    def logL(self, looptrace):
        return rouse.likelihood(self.trace, looptrace, self.model, **self.config['_logL_kwargs'])
